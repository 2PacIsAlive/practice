...
class MyGraph:
    ...
    def singleSourceShortestPath(self, srcID):
        ...
        while not pq.isEmpty(): 
            (minKey, minDist) = pq.extractMin()     # effectively removes node from priority queue
            for edge, dist in self.adjList[minKey]: # loop through all edges minKey --> edge  
                if pq.hasKey(edge):
                    if dist+minDist < pq.get(edge):
                        pq.set(edge, dist+minDist)
                        d[edge] = dist+minDist
                        pi[edge] = minKey
        
        # loop through the map with distances to nodes to make sure unaccessible nodes are accounted for
        for i in range(0,self.n):
            try: d[i]
            except KeyError: d[i] = pq.Inf
        return (d,pi)
